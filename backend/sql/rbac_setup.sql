...
--------------------------------------------------------------------------------
-- 2) The grants table
--    Re-running a clean install? Uncomment the DROP first (DESTRUCTIVE — loses all grant rows):
--      DROP TABLE ols_app_access CASCADE CONSTRAINTS PURGE;
--------------------------------------------------------------------------------
CREATE TABLE ols_app_access (
  ...
  CONSTRAINT ols_app_access_ck_type  CHECK (resource_type IN ('SCREEN','SERVER','APP','DB','TABLE_CATEGORY','TABLE','SECTION','REGRESSION','RECONCILIATION')),
...
COMMENT ON COLUMN ols_app_access.resource_type  IS 'SCREEN | SERVER | APP | DB | TABLE_CATEGORY | TABLE | SECTION | REGRESSION | RECONCILIATION';
COMMENT ON COLUMN ols_app_access.resource_scope IS 'log_analytics | config_ops:group|cib|retail | infra_health | service_console | oracle_command_center[:<db>] | user_management | docs | docs_technical | (REGRESSION/RECONCILIATION: group|cib|retail or config_ops:<scope>)';
...
--------------------------------------------------------------------------------
-- MIGRATION (existing installs only) — run this if ols_app_access was created before
-- 'RECONCILIATION' was added to the resource_type CHECK. Without it, a RECONCILIATION
-- grant (the Data Reconciliation tab) is rejected with ORA-02290. Safe to re-run.
--   ALTER TABLE ols_app_access DROP CONSTRAINT ols_app_access_ck_type;
--   ALTER TABLE ols_app_access ADD  CONSTRAINT ols_app_access_ck_type
--     CHECK (resource_type IN ('SCREEN','SERVER','APP','DB','TABLE_CATEGORY','TABLE','SECTION','REGRESSION','RECONCILIATION'));
--------------------------------------------------------------------------------
