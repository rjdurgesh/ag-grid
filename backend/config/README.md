...
| `refresh_databases` | this scope's **refreshable DBs**, grouped by category, driving the Refresh-DB grouped multi-select. **Env-aware:** key it by environment to serve DEV *and* STG from one file — `{ "DEV": { "BATCH": ["OLS_CIB_BATCH_DEV",…], "REPORTING": […] }, "STG": { "BATCH": ["OLS_CIB_BATCH_STG",…], … } }` — the backend returns the section matching **`APP_ENV`** (LIVE⇄PROD interchangeable; no section for this env → empty picker). Backward-compatible: a plain grouped object `{ "BATCH": [...], "REPORTING": [...] }` (or a flat array of names / `{key,label,category}`) with no env keys still works and applies to every env. |
