...
| `POST /api/oracle_cc/{db}/actions` | `{}` | **Action History** panel (Section 6d, after Materialized Views): recent gather-stats / MV-refresh runs — type, object, method, **by**, status chip, submitted, duration, error (clob). `summary:{actions,running}`. Reads `database.fetch_occ_actions` (`ols_occ_action_log`, newest first). |
