...
Tab details: Both the target `POST /api/access/admin/user` lookup and each `POST /api/access/admin/ops`
...
| `POST /api/oracle_cc/{db}/top_segments` | `{}` | Section 2 — top-10 tables by **data-segment** bytes as a real **3-level tree** (Table → Partition → Subpartition via `__children`), each node with its own stale-stats chip. `database.fetch_top_segments` runs 4 scoped queries → `{tables, stats, partitions, subpartitions}`: partition size = its own `TABLE PARTITION` segment **or**, for composite tables (no partition-level segment), the roll-up of its `TABLE SUBPARTITION` segments (`dba_tab_subpartitions` maps subpartition→parent partition, since `dba_segments` has no parent-partition column); subpartitions are the top-N per partition. Stats are keyed `(table, partition, subpartition)` so each level gets its chip. |
...
| `POST /api/oracle_cc/{db}/mviews` | `{}` | Section 6c — **materialized views** in the monitored schema (`DBA_MVIEWS`), one row per MV: name, last refresh (date + type), refresh method/mode, **Compile** chip (VALID/other), **Staleness** chip (FRESH/STALE/UNUSABLE/NEEDS_COMPILE → ok/warn/crit tint). Each row force-refreshable (`__actions:['mv_force','mv_complete','mv_fast']`) — WRITE-gated. `summary:{mviews,stale}`. Placed **after Temp, before Sessions Detail** (Sessions + SQL Intelligence stay last). Massages `database.fetch_mviews`. |
| `POST /api/oracle_cc/{db}/mview-refresh` | `{ owner, mview, method, caller }` (`method`: `complete`\|`fast`\|`force`) | **WRITE — ASYNC.** DB-write gated in the UI (same as Kill) + confirm; the user **chooses the method** (buttons **Force / Full / Incremental**). **Submits a background job** and returns immediately `{ action_id, state:'RUNNING' }` (a complete refresh can run for minutes) — does NOT block. Runs **`ols_util.occ_submit_mv_refresh`** (which creates a one-off DBMS_SCHEDULER job → `occ_run_action` → `DBMS_MVIEW.REFRESH`, method → code `C`\|`F`\|`?`) on a **privileged** connection (`_priv_cfg` → `app.state.sql_db_configs`, falls back to the monitor with a warning). The job records status/duration/error in **`ols_occ_action_log`**. Reference DDL: `backend/sql/occ_actions_setup.sql`. |
| `POST /api/oracle_cc/{db}/gather-stats` | `{ owner, table, caller }` | **WRITE — ASYNC.** DB-write gated + confirm; **submits a background job**, returns `{ action_id, state:'RUNNING' }`. Runs **`ols_util.occ_submit_gather`** → scheduler job → `occ_run_action` → `DBMS_STATS.gather_table_stats` at the granularity the proc picks (**subpartition+partition / partition / whole table**, cascade). Surfaced as a **"Gather stats" row action on the Session Details → Object Statistics panel** (rows carry `owner`/`table` + `__actions:['gather']`; `fetch_session_stats` now also selects `owner`). Logged in `ols_occ_action_log`. |
| `POST /api/oracle_cc/{db}/compress/object-info` | `{ table, owner? }` | **Section 2b — Object Compression Factory (READ).** Does the searched table exist + is it `partitioned`/`composite`? Drives which dropdowns show. Returns `{ found, partitioned, composite, partitioning_type, subpartitioning_type, owner, table, compress_types[] }` (`owner` defaults to `OCC_SCHEMA`). Reads `database.fetch_object_partitioning` (`dba_tables`/`dba_part_tables`). UI: capped-width search + type inputs, multi-select partitions (**latest 12** or `%2025%` search) with **Clear**; composite subpartitions show a few inline + **Show all N** → a **picker modal** (search / Select all / Clear / multi-select). Selections reset after a successful submit. |
| `POST /api/oracle_cc/{db}/compress/partitions` | `{ table, owner?, search?, limit? }` | Partitions for the dropdown — **latest `limit` (UI sends 12)**, or names matching `search` (a `%2025%` wildcard, else a 'contains'), capped at `limit`. `database.fetch_partitions` (`dba_tab_partitions`, `partition_position DESC`). |
| `POST /api/oracle_cc/{db}/compress/subpartitions` | `{ table, owner?, partitions[], search?, limit? }` | Subpartitions of the selected parent `partitions` (composite tables) — latest or by `search`. `database.fetch_subpartitions` (`dba_tab_subpartitions`). Empty `partitions` → `[]`. |
| `POST /api/oracle_cc/{db}/compress/run` | `{ table, owner?, compress_type, targets:[{partition, subpartition?}], caller }` | **WRITE — ASYNC.** DB-write gated (same as Kill) + confirm; **one background job per target**. `compress_type` ∈ `BASIC\|ADVANCED\|QUERY LOW\|QUERY HIGH\|ARCHIVE LOW\|ARCHIVE HIGH` (validated). Runs **`ols_util.occ_submit_compress`** → scheduler job → `occ_run_action` → `ALTER TABLE … MOVE [SUB]PARTITION … <clause> UPDATE INDEXES` (the proc maps the type → ROW/COLUMN STORE clause and `DBMS_ASSERT`-quotes identifiers) on a **privileged** connection. Returns `{ submitted:[{partition, subpartition?, action_id, state}\|{…, error}], message }`; the UI polls each `action_id` via `action-status`. Reference DDL: `backend/sql/occ_actions_setup.sql` (`COMPRESS` action_type + partition/subpartition columns). |
| `POST /api/oracle_cc/{db}/action-status` | `{ action_id }` | Live status of one submitted action (the UI **polls** this every ~3s until `state` != RUNNING, capped ~2 min → then falls back to Action History). Returns the action view incl. `state` (RUNNING\|SUCCESS\|FAILED\|UNKNOWN), `duration`, `error`. On SUCCESS the UI toasts + reloads the MV list; on FAILED it opens the error popup with the ORA text. Reads `database.fetch_occ_action`. |
| `POST /api/oracle_cc/{db}/actions` | `{}` | **Action History** panel (Section 6d, after Materialized Views): recent gather-stats / MV-refresh / **compress** runs — type, object (compress shows `owner.table:partition[:subpartition]`), method (compress: the type), **by**, status chip, submitted, duration, error (clob). `summary:{actions,running}`. Reads `database.fetch_occ_actions` (`ols_occ_action_log`, newest first). |
| `POST /api/oracle_cc/{db}/sessions` | `{ status }` (`active`\|`inactive`\|`killed`\|`all`, default `active`) | Section 7 — session inventory filtered by state. **The SQL lives in a DB proc** — `database.fetch_sessions` calls **`ols_util.occ_sessions(p_status, p_row_cap, p_rows OUT SYS_REFCURSOR, p_counts OUT SYS_REFCURSOR)`** (DDL: `backend/sql/occ_sessions_setup.sql`), so a DBA can tune it / add-remove columns **without an app redeploy**. Still **self-describing / column pass-through**: columns come from the ref-cursor's `description`, and `_sessions_payload` renders **every** column (label prettified; render overrides: `status`→chip, `sql_text`→clob, and **identifiers/hashes** — sid, serial#, parent, waiting_for, sql_id/prev_sql_id, plan_hash_value, sql_hash_value, audsid, user#, sql_exec_id, child numbers, row_wait_obj# — forced to `mono` so they show **as-is with no thousands-separator commas**; real quantities stay `num` and keep grouping) — add a column in the proc and it auto-displays, no Angular change. Kill/Deep-dive/chip still work via injected row metadata (`sid`, `serial`, `__actions` [`detail` always, `kill` unless KILLED], `status__sev`; KILLED rows `__sev:'crit'`). The rich query joins `v$sql`/`v$px_session`/`v$sessmetric`/`v$transaction` and adds parallel/PL-SQL/wait/resource/rank columns. **Two OPTIONAL lookups** — the OLS user-name table and the batch-job table — are resolved **inside the proc**: the candidate names are owned by the proc (`ols_util.occ_first_existing_table` picks the first that exists + is visible, handling `ols.ols_users` vs `psn_users` per DB), else the column is NULL and the batch join is dropped (a missing table never breaks the query). **No row cap** — every session matching the filter is returned. `summary:{active,inactive,killed,total}` (full counts regardless of filter, for the tab badges). |
