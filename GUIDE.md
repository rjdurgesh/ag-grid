...
| `POST /api/oracle_cc/{db}/compress/object-info` | `{ table, owner? }` | **Section 2b — Object Compression Factory (READ).** Does the searched table exist + is it `partitioned`/`composite`? Drives which dropdowns show. Returns `{ found, partitioned, composite, partitioning_type, subpartitioning_type, size_gb, owner, table, compress_types[] }` (`owner` defaults to `OCC_SCHEMA`; `size_gb` = total segment size shown in the meta line). Reads `database.fetch_object_partitioning` (`dba_tables`/`dba_part_tables` + `dba_segments` SUM). UI: capped-width search + type inputs, multi-select partitions (**latest 12** or `%2025%` search) with **Clear**; composite subpartitions render **one block PER selected partition** (each loaded independently via `compress/subpartitions` with that single partition) — a few inline + **Show all N** → a **per-partition picker modal** (search / Select all / Clear / multi-select, scoped to that partition; portaled to `<body>` + page-scroll-locked + both-axis scroll so it always overlays cleanly regardless of ancestor stacking). Selections are keyed `partitionsubpartition` so per-partition select/clear is exact; all selections reset after a successful submit. |
