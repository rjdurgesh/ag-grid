...
export class OlsCibRegressionComponent implements OnInit {
  ...
  // real DB name/SID is correct per environment; NOT hardcoded here. {key,label,name}. Used by Apply,
  // and (via dbDisplay) by Reset/Trigger too.
  ...
  readonly cleanupExamples: { label: string; json: string }[] = [
    { label: 'Delete every file in a folder', json: '{ "path": "D:\\\\ols\\\\cib\\\\logs" }' },
    { label: 'Only certain file types', json: '{ "path": "D:\\\\ols\\\\cib\\\\extracts", "include_pattern": ".csv,.dat" }' },
    { label: 'Recurse & drop emptied subfolders', json: '{ "path": "D:\\\\ols\\\\cib\\\\tmp", "include_subdir": "Y", "remove_empty_dir": "Y" }' },
    { label: 'Keep a few, delete the rest', json: '{ "path": "D:\\\\ols\\\\cib\\\\out", "include_pattern": "*", "exclude_pattern": ".keep,.gitkeep" }' },
    { label: 'Only files older than 30 days', json: '{ "path": "D:\\\\ols\\\\cib\\\\archive", "older_than_days": 30 }' }
  ];
  ...
  readonly fileCopyExamples: { label: string; json: string }[] = [
    { label: 'Copy one file', json: '{ "source": "D:\\\\rel\\\\ols_cib.jar", "destination": "D:\\\\ols\\\\cib\\\\lib\\\\ols_cib.jar" }' },
    { label: 'Copy a whole folder (recursive)', json: '{ "source": "D:\\\\rel\\\\config", "destination": "D:\\\\ols\\\\cib\\\\config" }' },
    { label: 'From a network share (UNC)', json: '{ "source": "\\\\\\\\eurv12\\\\d$\\\\rel\\\\rates.dat", "destination": "D:\\\\ols\\\\cib\\\\data\\\\rates.dat" }' }
  ];
  ...
  /** This scope's databases (CIB), derived from `databases` — the Reset / Trigger targets. */
  readonly resetTriggerDbs = computed(() => this.databases().filter((d) => d.key.startsWith('cib')));
