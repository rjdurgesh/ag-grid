...
export interface FileCopyStreamHandlers {
  item: (r: FileCopyResult, done: number, total: number) => void;  // one item finished (+ progress)
  step: (status: string) => void;       // overall step status (complete | partial | error)
  done: () => void;
  error: (e: unknown) => void;
}
...
export class OlsCibRegressionService {
  private readonly api = inject(ApiDataService);
  private readonly rbac = inject(RbacService);
  private readonly scope = 'cib';   // this app's regression stream (separate from other scopes)
  ...
  /** Read one chosen manifest (by repo path) → its items. */
